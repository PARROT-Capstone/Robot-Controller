...
int Planner::plan()
{
    ...
    this->task_planner->assign_pallets_to_robots(this->robot_poses, this->pallet_and_goal_poses);
    std::vector<int> task_assignments = this->task_planner->get_task_assignments();
    ...
    for (int i = 0; i < this->num_robots; i++)
    {
        ...
        if (this->motion_planners[i]->plan() == -1)
        {
            ...
            // unassign the task from this robot
            this->task_planner->unassign_pallet_from_robot(i);
            // update the task assignments
            task_assignments = this->task_planner->get_task_assignments();
